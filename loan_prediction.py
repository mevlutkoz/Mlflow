...
X_train, y_train, x_test, y_test = train_test_split(x, y, test_size = 0.3, random_state = RANDOM_SEED)
...
def eval(actual, pred):
    ...
    f1 = metrics.f1_score(actual, accuracy)
    fpr, tpr, _ = metrics.roc_curve(actual, pred)
    auc = metrics.auc(fpr,tpr)

    plt.figure(figsize=(8,8))
    plt.plot(fpr, tpr, color = "blue", label = "ROC curve area = %0.3f"%auc)
    plt.plot([0,1],[0,1], "r--")
    plt.xlim([-0.1, 1.1])
    plt.ylim([-0.1,1.1])
...
def mlflow_logging(model, x, y, name):
    ...
    #Logging best parameters from gridsearch method 
    #Logging metrics
    #Logging models
    ...
    with mlflow.start_run() as run:
        ...
        #Logging the params
        mlflow.log_params(model.best_params_) #GridSearchCV object's method
        #Logging the metrics
        ...
        #Logging the artifacts and model
        #Logging the directory that plots saved
        mlflow.log_artifact("plots/ROC_curve.png")
        #Logging the model with name
        mlflow.sklearn.log_model(model, name)

        mlflow.end_run()

Her bir model için Load_Prediction adı altında, farklı runlar oluşturacak, mlflow.set_tag("run_id", run_id) ile dinamik
    tag vericek. For each model which is same as the each run in this case we will have logged parameters, logged metrics,
    logged model, and also logged plots(artifacts)
